% visualize imagenet-vgg-verydeep-19 neural network
...
if ~isfield(net, 'normalization')
    name_imdb = '/Users/chuan/Research/library/matconvnet/data/imagenet/imagenet-vgg-verydeep-19.mat';
    imdb = load(name_imdb);
    ...
    save('/Users/chuan/Research/library/matconvnet/data/imagenet/imagenet-vgg-verydeep-19.mat', 'net');
end
...
for i_layer = 1:size(net.layers, 2)
    if strcmp(net.layers{1, i_layer}.type, 'conv') && net.layers{1, i_layer}.pad(1) ~= 0
        ...
        if  strcmp(net.layers{1, i_layer}.type, 'pool')
            scale = scale * net.layers{1, i_layer}.stride(1);
        end
    end
end
...
for i_img = 1:num_img
    ...
    name_img = ['/Users/chuan/Research/Dataset/data/imagenet/ILSVRC2012_img_val/ILSVRC2012_val_' sprintf('%08d', i_img) '.JPEG'];
    ...
    if size(x_ini, 3) == 1
        x_ini = cat(3, x_ini, x_ini, x_ini);
    end
    
    % rescale the image to a pre-defined size
    size_input = size(x_ini);
    scale_input = net.normalization.imageSize(1) / min(size_input(1:2));
    x_ini = imresize(x_ini, scale_input);
...
save(name_output_model, 'net', '-v7.3');
