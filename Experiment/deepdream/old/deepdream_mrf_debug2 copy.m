...
for i_img = 1:num_img
    ...
    all_response = [all_response; reshape(res(end).x, size(res(end).x, 3), [])];
end

step = 10;
figure;
hold on;
for i = 1:step:size(all_response, 1)
    plot([1:size(all_response, 2)], all_response(i, :), 'b');
end
axis([0 size(all_response, 2) -1500 1500]);
